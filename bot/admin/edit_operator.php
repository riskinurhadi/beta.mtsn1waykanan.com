...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check = "SELECT id FROM operator_madrasah WHERE (username = ? OR email = ?) AND id != ?";
    $stmt_check = $conn->prepare($sql_check);
    $stmt_check->bind_param("ssi", $username, $email, $id);
    $stmt_check->execute();
    $stmt_check->store_result();
    if ($stmt_check->num_rows > 0) {
        $errors[] = "Username atau Email sudah digunakan oleh operator lain.";
    }
    $stmt_check->close();
    ...
    if (empty($errors)) {
        ...
        // Tambahkan password ke query jika diubah
        if (!empty($password)) {
...
$sql_select = "SELECT nama_lengkap, username, email, role, status, foto_profil FROM operator_madrasah WHERE id = ?";
