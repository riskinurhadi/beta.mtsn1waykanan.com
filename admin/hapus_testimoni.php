...
if (isset($_GET['id']) && filter_var($_GET['id'], FILTER_VALIDATE_INT)) {
    ...
    $sql_select = "SELECT foto_alumni FROM testimoni_alumni WHERE id = ?";
    if ($stmt_select = $conn->prepare($sql_select)) {
        $stmt_select->bind_param("i", $id);
        $stmt_select->execute();
        $stmt_select->bind_result($foto_alumni);
        
        // Simpan nama file jika ada
        if ($stmt_select->fetch()) {
            $file_foto_untuk_dihapus = $foto_alumni;
        }
        $stmt_select->close();
    }

    // Jika data testimoni dengan ID tersebut ada, lanjutkan proses hapus
    if ($file_foto_untuk_dihapus !== null) {
        // 3. Hapus record dari tabel 'testimoni_alumni' di database
        $sql_delete = "DELETE FROM testimoni_alumni WHERE id = ?";
        if ($stmt_delete = $conn->prepare($sql_delete)) {
            ...
            if ($stmt_delete->execute()) {
                ...
                if ($file_foto_untuk_dihapus && $file_foto_untuk_dihapus !== 'default-profile.png') {
                    $file_path = 'uploads/testimoni/' . $file_foto_untuk_dihapus;
                    if (file_exists($file_path)) {
                        unlink($file_path); // Fungsi PHP untuk menghapus file
                    }
                }
                // Siapkan pesan sukses untuk notifikasi
                $_SESSION['success_message'] = "Data testimoni berhasil dihapus.";
            } else {
