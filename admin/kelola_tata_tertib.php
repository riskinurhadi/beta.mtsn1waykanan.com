...
// --- AMBIL DATA TATA TERTIB DARI DATABASE ---
$tata_tertib_list = [];
...
<head>
    ...
    <title>Kelola Tata Tertib - Admin</title>
...
<body>
    ...
    <main class="main-content">
        <header class="page-header">
            <h1>Kelola File Tata Tertib</h1>
            <div class="header-actions">
                <div class="search-container">
                    <i class="fas fa-search"></i>
                    <input type="text" id="searchInput" onkeyup="searchTable()" placeholder="Cari berdasarkan judul...">
                </div>
                <a href="tambah_tata_tertib.php" class="btn-add"><i class="fas fa-plus"></i> Unggah File Baru</a>
            </div>
        </header>

        <section class="content">
            <div class="table-container">
                <table class="data-table" id="tataTertibTable">
                    <thead>
                        <tr>
                            <th>Judul</th>
                            <th>Nama File</th>
                    ...
                    <tbody>
                        <?php if (empty($tata_tertib_list)): ?>
                            <tr>
                                <td colspan="5" style="text-align: center;">Belum ada file tata tertib yang diunggah.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($tata_tertib_list as $item): ?>
                            <tr>
                                <td><strong><?php echo htmlspecialchars($item['judul']); ?></strong></td>
                                <td><a href="uploads/tata_tertib/<?php echo htmlspecialchars($item['nama_file']); ?>" target="_blank"><?php echo htmlspecialchars($item['nama_file']); ?></a></td>
                                <td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <?php if (!$item['is_active']): ?>
                                            <a href="aktifkan_tata_tertib.php?id=<?php echo $item['id']; ?>" class="btn-action btn-activate" title="Jadikan Aktif"><i class="fas fa-check-circle"></i> Aktifkan</a>
                                        <?php endif; ?>
    ...
    <script>
        function searchTable() {
            ...
            table = document.getElementById("tataTertibTable");
            tr = table.getElementsByTagName("tr");

            for (i = 1; i < tr.length; i++) {
                tdJudul = tr[i].getElementsByTagName("td")[0];
                if (tdJudul) {
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                text: "File tata tertib ini akan dihapus secara permanen!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    window.location.href = 'hapus_tata_tertib.php?id=' + id;
                }
            });
        }
