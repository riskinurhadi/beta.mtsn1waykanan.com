...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $is_active = isset($_POST['is_active']) ? 1 : 0; // 1 jika dicentang, 0 jika tidak
    ...
    if (isset($_FILES['nama_file']) && $_FILES['nama_file']['error'] == 0) {
        ...
        if (!in_array($file_ext, $allowed_types)) { $errors[] = "Format gambar tidak diizinkan. Hanya JPG, JPEG, dan PNG."; }
        if ($_FILES['nama_file']['size'] > $max_file_size) { $errors[] = "Ukuran gambar tidak boleh lebih dari 5 MB."; }
    } else {
        $errors[] = "Anda wajib memilih file gambar untuk diunggah.";
    }

    // 3. Jika tidak ada error, proses data
    if (empty($errors)) {
        // Jika gambar baru akan diaktifkan, nonaktifkan semua yang lain terlebih dahulu
        if ($is_active) {
            $conn->query("UPDATE gambar_struktur SET is_active = FALSE");
        }
        ...
        if (move_uploaded_file($_FILES['nama_file']['tmp_name'], $target_file)) {
            ...
            $sql = "INSERT INTO gambar_struktur (judul, nama_file, is_active) VALUES (?, ?, ?)";
            
            if ($stmt = $conn->prepare($sql)) {
                $stmt->bind_param("ssi", $judul, $new_file_name, $is_active);
                if ($stmt->execute()) {
                    $_SESSION['success_message'] = "Gambar struktur baru berhasil diunggah!";
            ...
            $errors[] = "Gagal mengunggah gambar. Pastikan folder 'uploads/struktur' ada dan bisa ditulis.";
        }
    }
    $conn->close();
}
...
<head>
    ...
    <title>Unggah Gambar Struktur - Admin</title>
...
<body>
    ...
    <main class="main-content">
        <header class="page-header">
            <h1>Unggah Gambar Struktur Baru</h1>
        ...
        <section class="content">
            <div class="form-card">
                ...
                <form action="tambah_gambar_struktur.php" method="post" enctype="multipart/form-data">
                    <div class="form-group">
                        <label for="judul">Judul Gambar</label>
                        <input type="text" id="judul" name="judul" placeholder="Contoh: Struktur Organisasi 2024/2025" required value="<?php echo htmlspecialchars($judul); ?>">
                    </div>
                    
                    <div class="form-group">
                        <label for="nama_file">Pilih File Gambar</label>
                        <input type="file" id="nama_file" name="nama_file" required accept="image/png, image/jpeg, image/jpg">
                        <small style="color: #777; margin-top: 5px; display: block;">Format: JPG, PNG. Ukuran maks: 5 MB.</small>
                    </div>

                    <div class="form-group checkbox-group">
                        <input type="checkbox" id="is_active" name="is_active" value="1">
                        <label for="is_active" style="margin-bottom: 0;">Jadikan gambar ini aktif?</label>
                    </div>
