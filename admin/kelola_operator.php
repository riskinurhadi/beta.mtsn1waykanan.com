...
// --- AMBIL DATA OPERATOR DARI DATABASE ---
$operators = [];
...
<head>
    ...
    <style>
        :root{--primary-color:#28a745;--primary-hover:#218838;--sidebar-bg:#2c3e50;--sidebar-text:#ecf0f1;--sidebar-active:#34495e;--main-bg:#f4f7f6;--text-color:#333;--card-shadow:0 4px 15px rgba(0,0,0,.08);--danger-color:#e74c3c;--danger-hover:#c0392b;--info-color:#3498db;--warning-color:#f39c12;--secondary-color:#6c757d; --developer-color: #007bff;}
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .profile-pic-thumb{width:45px;height:45px;border-radius:50%;object-fit:cover;border:2px solid #eee}
        ...
        .badge-developer{background-color:var(--developer-color)}
        ...
        .btn-notify{background-color:var(--info-color)}
...
<body>
        ...
        <header class="page-header">
            ...
            <div class="header-actions">
                <div class="search-container">
                    ...
                    <input type="text" id="searchInput" onkeyup="searchTable()" placeholder="Cari nama atau username...">
                </div>
                <a href="tambah_operator.php" class="btn-add"><i class="fas fa-user-plus"></i> Tambah Operator</a>
            </div>
        </header>
                ...
                <table class="data-table" id="operatorTable">
                    ...
                    <tbody>
                        <?php if (empty($operators)): ?>
                            <tr>
                                <td colspan="8" style="text-align: center;">Belum ada data operator.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($operators as $index => $op): ?>
                            <tr>
                                <td><?php echo $index + 1; ?></td>
                                <td><img src="uploads/operator/<?php echo htmlspecialchars($op['foto_profil'] ?? 'default-profile.png'); ?>" alt="Foto Profil" class="profile-pic-thumb"></td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <?php if ($op['id'] !== $_SESSION['operator_id']): ?>
                                            <a href="kirim_notifikasi.php?id=<?php echo $op['id']; ?>" class="btn-action btn-notify" title="Kirim Notifikasi"><i class="fas fa-paper-plane"></i></a>
                                        <?php endif; ?>
                                        <a href="edit_operator.php?id=<?php echo $op['id']; ?>" class="btn-action btn-edit" title="Edit"><i class="fas fa-pencil-alt"></i></a>
                                        <?php if ($op['id'] !== $_SESSION['operator_id']): ?>
                                            <button type="button" class="btn-action btn-delete" onclick="confirmDelete(<?php echo $op['id']; ?>)" title="Hapus"><i class="fas fa-trash"></i></button>
                                        <?php endif; ?>
                                    </div>
                                </td>
                            </tr>
    ...
    <script>
        function searchTable() {
            var input, filter, table, tr, tdNama, tdUsername, i, txtValueNama, txtValueUsername;
            input = document.getElementById("searchInput");
            filter = input.value.toUpperCase();
            table = document.getElementById("operatorTable");
            tr = table.getElementsByTagName("tr");

            for (i = 1; i < tr.length; i++) {
                tdNama = tr[i].getElementsByTagName("td")[2];
                tdUsername = tr[i].getElementsByTagName("td")[3];
                if (tdNama || tdUsername) {
                    txtValueNama = tdNama.textContent || tdNama.innerText;
                    txtValueUsername = tdUsername.textContent || tdUsername.innerText;
                    if (txtValueNama.toUpperCase().indexOf(filter) > -1 || txtValueUsername.toUpperCase().indexOf(filter) > -1) {
                        tr[i].style.display = "";
                    } else {
