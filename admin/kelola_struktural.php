...
// --- AMBIL DATA GAMBAR STRUKTUR DARI DATABASE ---
$gambar_list = [];
$sql = "SELECT id, judul, nama_file, is_active, tanggal_upload 
        FROM gambar_struktur 
...
<head>
    ...
    <title>Kelola Gambar Struktur - Admin</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .struktur-thumb{width:150px;height:auto;border-radius:8px;border:2px solid #eee}
...
<body>
    ...
    <main class="main-content">
        <header class="page-header">
            <h1>Kelola Gambar Struktur Organisasi</h1>
            <div class="header-actions">
                <div class="search-container">
                    <i class="fas fa-search"></i>
                    <input type="text" id="searchInput" onkeyup="searchTable()" placeholder="Cari berdasarkan judul...">
                </div>
                <a href="tambah_gambar_struktur.php" class="btn-add"><i class="fas fa-plus"></i> Unggah Gambar</a>
            </div>
        </header>

        <section class="content">
            <div class="table-container">
                <table class="data-table" id="strukturTable">
                    <thead>
                        <tr>
                            <th>Preview</th>
                            <th>Judul</th>
                    ...
                    <tbody>
                        <?php if (empty($gambar_list)): ?>
                            <tr>
                                <td colspan="5" style="text-align: center;">Belum ada gambar struktur yang diunggah.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($gambar_list as $item): ?>
                            <tr>
                                <td><img src="uploads/struktur/<?php echo htmlspecialchars($item['nama_file']); ?>" alt="<?php echo htmlspecialchars($item['judul']); ?>" class="struktur-thumb"></td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <?php if (!$item['is_active']): ?>
                                            <a href="aktifkan_struktur.php?id=<?php echo $item['id']; ?>" class="btn-action btn-activate" title="Jadikan Aktif"><i class="fas fa-check-circle"></i> Aktifkan</a>
                                        <?php endif; ?>
    ...
    <script>
        function searchTable() {
            var input, filter, table, tr, tdJudul, i, txtValue;
            input = document.getElementById("searchInput");
            filter = input.value.toUpperCase();
            table = document.getElementById("strukturTable");
            tr = table.getElementsByTagName("tr");

            for (i = 1; i < tr.length; i++) {
                tdJudul = tr[i].getElementsByTagName("td")[1];
                if (tdJudul) {
                    txtValue = tdJudul.textContent || tdJudul.innerText;
        ...
        function confirmDelete(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'hapus_gambar_struktur.php?id=' + id;
                }
            });
        }
