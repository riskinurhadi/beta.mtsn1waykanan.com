...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validasi input
    if (empty($nama_alumni) || empty($testimoni)) {
        $errors[] = "Nama Alumni dan Isi Testimoni tidak boleh kosong.";
    }
    ...
    if (empty($errors)) {
        $sql = "UPDATE testimoni_alumni SET nama_alumni = ?, info_singkat = ?, testimoni = ?, foto_alumni = ?, is_tampil = ? WHERE id = ?";
        if ($stmt = $conn->prepare($sql)) {
            $stmt->bind_param("ssssii", $nama_alumni, $info_singkat, $testimoni, $new_file_name, $is_tampil, $id);
            if ($stmt->execute()) {
                $_SESSION['success_message'] = "Data testimoni berhasil diperbarui.";
                header("Location: kelola_testimoni.php");
                exit();
            } else {
...
$sql_select = "SELECT * FROM testimoni_alumni WHERE id = ?";
if ($stmt_select = $conn->prepare($sql_select)) {
    ...
    if ($result->num_rows == 1) {
        ...
        $_SESSION['error_message'] = "Data testimoni tidak ditemukan.";
        header("Location: kelola_testimoni.php");
        exit();
    }
    $stmt_select->close();
    ...
    <main class="main-content">
        ...
        <section class="content">
            <div class="form-card">
                ...
                <form action="edit_testimoni.php?id=<?php echo $id; ?>" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="<?php echo $id; ?>">
                    <input type="hidden" name="current_foto" value="<?php echo htmlspecialchars($testi['foto_alumni']); ?>">

                    <div class="form-group">
                        <label>Foto Saat Ini</label><br>
                        <img src="uploads/testimoni/<?php echo htmlspecialchars($testi['foto_alumni'] ?? 'default-profile.png'); ?>" alt="Foto saat ini" class="current-image">
                    </div>
                    <div class="form-group">
                        <label for="foto_alumni">Ganti Foto (Opsional)</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="nama_alumni" name="nama_alumni" required value="<?php echo htmlspecialchars($testi['nama_alumni']); ?>">
                    </div>
                    <div class="form-group">
                        <label for="info_singkat">Info Singkat (Opsional)</label>
                        <input type="text" id="info_singkat" name="info_singkat" placeholder="Contoh: Lulusan 2020, Mahasiswa UGM" value="<?php echo htmlspecialchars($testi['info_singkat']); ?>">
                    </div>
                    <div class="form-group">
                        <label for="testimoni">Isi Testimoni (Wajib)</label>
                        <textarea id="testimoni" name="testimoni" rows="5" required><?php echo htmlspecialchars($testi['testimoni']); ?></textarea>
                    </div>
                    <div class="form-group checkbox-group">
                        <input type="checkbox" id="is_tampil" name="is_tampil" value="1" <?php echo ($testi['is_tampil']) ? 'checked' : ''; ?>>
                        <label for="is_tampil" style="margin-bottom: 0;">Tampilkan di halaman publik?</label>
                    </div>
                    <button type="submit" class="btn-submit"><i class="fas fa-sync-alt"></i> Perbarui Testimoni</button>
                </form>
            </div>
        </section>
    </main>
